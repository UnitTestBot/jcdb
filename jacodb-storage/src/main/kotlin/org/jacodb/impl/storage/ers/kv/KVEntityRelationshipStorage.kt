...
class KVEntityRelationshipStorage(private val kvStorage: PluggableKeyValueStorage) : EntityRelationshipStorageBase() {
    ...
    internal fun getEntityTypeId(
        type: String,
        kvTxn: org.jacodb.api.storage.kv.Transaction
    ): Int? {
    ...
    private fun entityTypesMap(txn: org.jacodb.api.storage.kv.Transaction, create: Boolean): NamedMap? {
        return entityTypesMap ?: txn.getNamedMap(entityTypesMapName, create)?.also {
            entityTypesMap = it
        }
    }

    internal fun entityCountersMap(txn: org.jacodb.api.storage.kv.Transaction, create: Boolean): NamedMap? {
    ...
    internal fun deletedEntitiesMap(
        typeId: Int,
        txn: org.jacodb.api.storage.kv.Transaction,
        create: Boolean
    ): NamedMap? {
